...
def perform_task(input, what, how, times_to_repeat=1):
   ...
   if what.startswith("A"): # Need to build tree
     ...
     from octree import readOctree as readTree
   ...
   if what.endswith("BC"): # Need to compute moments
     ...
     if how=="old":
       from momentTree3D import octreeMoments as treeMoments
       moments = repeat_times(treeMoments, tree, times_to_repeat)
     elif how=="new":
       from momentTreeNew3D import octreeMoments as treeMoments
       from momentTreeNew3D import preprocessing
